...
include "sertifikat_oop.php";
$crud = new Sertifikat ("localhost", "root", "","db_alumni");
$pro="simpan";
...
<form id="form1" name="form1" method="post" action="">
  ...
  <select name="kursus" id="kursus">
	<option value="" name="kursus">Pilih Kursus</option> 
	<?php
$row = $crud->fetch("tb_kursus");
foreach ($row as $data) {
?>
		<option value="<?php echo $data['kd_kursus']; ?>"> <?php echo $data['nama_kursus']; ?></option>
...
<table width="956" class="table table-striped table-bordered table-hover">
  ...
  <tr bgcolor="#cccccc">
    ...
    <th width="41%"><center>
     ...
     <th width="46%"><div align="center">Kursus 
       </td>
       
     </div>
     <th width="10%"><div align="center">Nilai 
       </td>
  ...
  <tbody> 
<?php  
//$sql="select tb_peserta.nrp,tb_peserta.nama,tb_peserta.pangkat,tb_peserta.golongan,tb_peserta.alamat_kantor,tb_peserta.golongan,tb_peserta.golongan,tb_peserta.golongan, from `$tb_sertifikat` where status='Diterima' order by `kd_sertifikat` desc";
$sql="select * from tb_peserta,tb_nilai where tb_nilai.nrp=$nrp and tb_peserta.nrp=$nrp order by tb_nilai.nilai desc";
/*if (isset($_POST["Cari"])){
	...
	$sql= "select * from tb_peserta p
INNER JOIN tb_sertifikat s ON s.nrp = p.nrp
INNER JOIN tb_kursus k ON k.kd_kursus = s.kd_kursus
WHERE k.nama_kursus LIKE '%$kursus%' AND p.kota_kabupaten LIKE '%$kota_kabupaten%' AND p.provinsi LIKE '%$provinsi%'";
if($pilih=='nrp'){
	//$item = '1-11040012250981';
	$sql= "select * from tb_peserta p
INNER JOIN tb_sertifikat s ON s.nrp = p.nrp
INNER JOIN tb_kursus k ON k.kd_kursus = s.kd_kursus
INNER JOIN tb_nilai n ON n.nrp = p.nrp
WHERE p.nrp='$item' 
AND s.kd_kursus='$kursus'
AND n.kd_kursus='$kursus'
AND k.kd_kursus='$kursus'
AND s.status='Diterima' 
AND n.keterangan = 'Akhir'
order by n.nilai DESC";
} else if($pilih=='kota_kabupaten'){
	$sql= "select * from tb_peserta p
INNER JOIN tb_sertifikat s ON s.nrp = p.nrp
INNER JOIN tb_kursus k ON k.kd_kursus = s.kd_kursus
INNER JOIN tb_nilai n ON n.nrp = p.nrp
WHERE k.kd_kursus='$kursus' 
AND s.kd_kursus='$kursus'
AND n.kd_kursus='$kursus'
AND s.status='Diterima' 
AND n.keterangan = 'Akhir'
AND p.kota_kabupaten LIKE '%$item%'
order by n.nilai DESC";
} else if($pilih=='provinsi'){
...
AND s.status='Diterima' 
AND n.keterangan = 'Akhir'
AND p.provinsi LIKE '%$item%'
order by n.nilai DESC";
}
	// $sql="select * from tb_peserta,tb_sertifikat,tb_nilai 
	// where tb_nilai.nrp=tb_sertifikat.nrp 
	// and tb_nilai.nrp=tb_peserta.nrp 
	// and tb_peserta.nrp=tb_sertifikat.nrp 
	// and tb_sertifikat.status='Diterima' 
	// and tb_nilai.keterangan = 'akhir' 
	// and tb_peserta.$pilih like '%$item%' 
	// and tb_sertifikat.kd_kursus like '%$kursus%'  
	// order by tb_nilai.nilai desc";
	
	//$sql="select * from tb_peserta,tb_sertifikat where tb_peserta.nrp=tb_sertifikat.nrp and tb_sertifikat.status='Diterima' and tb_peserta.$pilih like '%$item%'";
	}*/
  $jum=getJum($conn,$sql);
		if($jum > 0){
	//--------------------------------------------------------------------------------------------
...
echo"<tr bgcolor='$color'>
				...
				<td><a href='index.php?mnu=knilai_detail&kd=$kd_nilai'>$nilai
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=rekam_data'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=rekam_data'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=rekam_data'>Next »</a></span>";
	}
...
$simpan=process($conn,$sql);
		if($simpan) {echo "<script>alert('Data $kd_sertifikat berhasil disimpan !');document.location.href='?mnu=rekam_data';</script>";}
		else{echo"<script>alert('Data $kd_sertifikat gagal disimpan...');document.location.href='?mnu=rekam_data';</script>";}
	}
	else{
...
$ubah=process($conn,$sql);
	if($ubah) {echo "<script>alert('Data $kd_sertifikat berhasil diubah !');document.location.href='?mnu=rekam_data';</script>";}
	else{echo"<script>alert('Data $kd_sertifikat gagal diubah...');document.location.href='?mnu=rekam_data';</script>";}
	}//else simpan
}
?>
...
if($hapus) {echo "<script>alert('Data sertifikat $kd_sertifikat berhasil dihapus !');document.location.href='?mnu=rekam_data';</script>";}
else{echo"<script>alert('Data sertifikat $kd_sertifikat gagal dihapus...');document.location.href='?mnu=rekam_data';</script>";}
}
?>
