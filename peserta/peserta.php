...
<option value="Jenderal"  <?php if($pangkat=="Jenderal"){echo"selected";} ?>>Jenderal</option>
    ...
    <td width="545" align="left" valign="top"><table width="496" align="left">
     ...
     <td height="44"><label for="pendidikan_umum2">Pendidikan Umum Terakhir</label></td>
      ...
      <tr>
        <td height="43"><label for="telepon2">Telepon</label></td>
        <td>:</td>
        <td colspan="2"><input class="form-control"  name="jurusan" type="text" id="telepon2" value="<?php echo $telepon;?>" size="35" /></td>
      </tr>   
  ...
  <td height="53"><label>Gambar</label>
    <td>:<td colspan="2"><label for="gambar"></label>
      ...
      <tr>
        <td height="40">&nbsp;</td>
        <td>&nbsp;</td>
        <td colspan="2"><input name="Simpan" type="submit" id="Simpan" onclick="MM_validateForm('nrp','','R','nama2','','R','tempat_lahir2','','R','tanggal_lahir','','R','jabatan2','','R','kota_kabupaten2','','R','provinsi2','','R','telepon_kantor2','','R','telepon2','','','R','alamat_kantor2','','R','alamat_rumah2','','R');return document.MM_returnValue" value="Simpan" />
...
<table></table>
  ...
  <?php  
  $sqlq="select distinct(golongan) from `tb_peserta` order by `nrp` desc";
...
?>     
   <h2><a href="#">Data Personel <?php echo"$golongan";?></a></h2>
...
<table width="1079" class="table table-striped table-bordered table-hover">
  ...
  <tbody>
<?php  
  $sql="select * from `tb_peserta`  where golongan='$golongan' order by `nrp` desc";
  $jum=getJum($conn,$sql);
		if($jum > 0){
	//--------------------------------------------------------------------------------------------
	$batas   = 15;
...
echo"<tr bgcolor='$color'>
				<td>$no</td>
				<td>Nama :$nrp - $nama <br>
				Pangkat/ golongan :  $pangkat. $korps<br>
				...
				<td align='center'>
<a href='?mnu=personel&pro=ubah&kode=$nrp'><img src='ypathicon/u.png' alt='ubah'></a>
<a href='?mnu=personel&pro=hapus&kode=$nrp'><img src='ypathicon/h.png' alt='hapus' 
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=personel'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=personel'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=personel'>Next »</a></span>";
	}
...
$simpan=process($conn,$sql);
		if($simpan) {echo "<script>alert('Data $nrp berhasil disimpan !');document.location.href='?mnu=personel';</script>";}
		else{echo"<script>alert('Data $nrp gagal disimpan...');document.location.href='?mnu=personel';</script>";}
	}
	else{
...
$ubah=process($conn,$sql);
	if($ubah) {echo "<script>alert('Data $nrp berhasil diubah !');document.location.href='?mnu=personel';</script>";}
	else{echo"<script>alert('Data $nrp gagal diubah...');document.location.href='?mnu=personel';</script>";}
	}//else simpan
}
?>
...
if($hapus) {echo "<script>alert('Data peserta $nrp berhasil dihapus !');document.location.href='?mnu=personel';</script>";}
else{echo"<script>alert('Data peserta $nrp gagal dihapus...');document.location.href='?mnu=personel';</script>";}
}
?>
