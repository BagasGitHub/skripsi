...
<tr>
<td height="53"><label for="kd_kursus_dibuka">Nama Sub Kursus</label>
<td>:
...
<tr>
<td height="38"><label for="tanggal_daftar">Tanggal Lulus</label>
...
<td height="40"><label for="no_sprint">Nomor Sertifikat</label></td>
        <td>:</td>
...
<td colspan="2">	<input name="Simpan" type="submit" id="Simpan" onclick="MM_validateForm('nrp','','R','tanggal_daftar','','R');return document.MM_returnValue" value="Simpan" />
        ...
        <a href="?mnu=pendataan"><input name="Batal" type="button" id="Batal" value="Batal" /></a>
</td></tr>
</table>
</form>
...
?>     
   ...
   <h2><a href="#">Data Alumni Bahasa <?php echo"$NB";?></a></h2>
     <div>

Data Pendataan <?php echo $NB;?>: 
 ...
 <table width="100%" class="table table-striped table-bordered table-hover">
  ...
  <tr bgcolor="#cccccc">
    <th width="4%"><center>No</th>
       <th width="29%"><center>Alumni</th>
    <th width="31%"><div align="center">Lulus Pada</div></th>
...
echo"<tr bgcolor='$color'>
				...
				<td>NRP/NIP : $nrp 
				<br>Nama    : $nrp0  
				<td>Kursus: $kd_kursus_dibuka
				<br>Nomor Sertifikat: $no_sprint 
				
				<br> Tanggal Lulus:$tanggal_daftar </td>
				<td align='center'>
<a href='?mnu=pendataan&pro=ubah&kode=$kd_pendaftaran'><img src='ypathicon/u.png' alt='ubah'></a>
<a href='?mnu=pendataan&pro=hapus&kode=$kd_pendaftaran'><img src='ypathicon/h.png' alt='hapus' 
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=pendataan'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=pendataan'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=pendataan'>Next »</a></span>";
	}
...
$simpan=process($conn,$sql);
		if($simpan) {echo "<script>alert('Data $kd_pendaftaran berhasil disimpan !');document.location.href='?mnu=pendataan';</script>";}
		else{echo"<script>alert('Data $kd_pendaftaran gagal disimpan...');document.location.href='?mnu=pendataan';</script>";}
	}
	else{
...
$ubah=process($conn,$sql);
	if($ubah) {echo "<script>alert('Data $kd_pendaftaran berhasil diubah !');document.location.href='?mnu=pendataan';</script>";}
	else{echo"<script>alert('Data $kd_pendaftaran gagal diubah...');document.location.href='?mnu=pendataan';</script>";}
	}//else simpan
}
?>
...
if($hapus) {echo "<script>alert('Data pendataan $kd_pendaftaran berhasil dihapus !');document.location.href='?mnu=pendataan';</script>";}
else{echo"<script>alert('Data pendataan $kd_pendaftaran gagal dihapus...');document.location.href='?mnu=pendataan';</script>";}
}
?>
